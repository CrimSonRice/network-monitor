...
python -m ui.monitor_tui ... --ssh-interval 600 --ssh-port 22  # SSH check (default 600s / 10 min, port 22)
```

**Header:** Run time (session age), total/online/offline, No SSH count, latency min/avg/max (UP hosts), status flips in window, and alert history count.

**SSH column:** Shows whether each host’s SSH port (default 22) is open (OK) or not (NO). Check runs every 10 min by default to avoid rate limits.

**Filters (Windows, while running):** Type to filter; use **comma or space** to search multiple IPs at once (e.g. `8.8.8.8, 1.1.1.1`). **Esc** clears. **u** = UP + SSH OK, **d** = DOWN or no SSH, **a** = all.
...
Optional env: `PING_INTERVAL`, `WINDOW_SIZE`, `REFRESH_RATE`, `PAGE_SIZE`, `SSH_CHECK_INTERVAL` (default 600), `SSH_PORT`. One ping thread handles all targets (scales to 300+). Press **Ctrl+C** to exit.
